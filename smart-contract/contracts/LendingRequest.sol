...
contract LendingRequest {
    ...
    uint256 public amountAsked; // Amount of tokens to be asked for
    uint256 public paybackAmount; // Amount of tokens being promised to be returned
    string public purpose; // Purpose of asking this loan
    uint256 public collateralCollectionTimestamp; // Time after which collateral can be collected by lender if loan not
    ...
    bool public collateralCollected; // Global state showing whather collateral was collected by lender if loan not settled
    ...
    /**
     * @notice Transfer the amount of tokens being asked from Lender's to asker's account.
    ...
    function lend(address payable _lender) external onlyOwner returns (bool success) {
        ...
        checks:
            ...
            must not have been defaulted & collateral collected already
            must not be sponsored by the lender themeselves
     ...
     * @notice Also return any collateral deposited by asker back to there account
     * @param _asker the address of initial caller of the function
     ...
     * @return success returns true on success otherwise false
     */
    function collectCollateral(address payable _lender) external onlyOwner returns (bool success) {
        ...
        checks :
            ...
            Deadline for payback must passed
        */
    ...
    /**
     * @notice cancels request if possible
     */
